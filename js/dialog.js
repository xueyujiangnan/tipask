...
$._dialog = {
	...
	init : function(){
		...
		self.skins['default']={
			...
			'alphaDiv':'z-index:100;cursor:default;background-color:#FFFFFF;'
		...
		if(options.minScrollTop){
			...
			if(options.effects && typeof options['isLoad'] == 'undefined'){
				...
				switch(options.effects){
					case 'down':
						dialog.css({left:pos.left, top:-100,display:'block',opacity:0.1}).animate({top:top,opacity:1},500, function(){dialog.css({opacity:''})} );
					break;
					case 'up':
	...
	build : function(options){
		...
		if (!dialog){
			...
			+ 'z-index:2010;'+position+';left:-1000px;top:-1000px;">'
			...
			if(typeof options.fnOk == 'function' || typeof options.fnCancel == 'function' ){
				...
				if(typeof options.fnOk == 'function'){
					html +='<input type="button" class="button5" name="ok" value="'+options.okText+'" />'
				}
				if(typeof options.fnCancel == 'function'){
					html +='<input type="button"  class="button5" name="cancel" value="'+options.cancelText+'" style="margin-left:20px;"/>'
				}
				html +='</td></tr>'
			}
	...
	close : function(id){
		...
		if (dialog && self.isShow(id)){
			...
			function fn_dialogClose(){
				dialog.css({top:-1100, opacity:''});
				self.hideOverlay();
			}
			var o_dialogClose={duration:200, complete:fn_dialogClose};
			switch(options.effects){
				case 'down':
					dialog.animate({top:-1100, opacity:0.1},o_dialogClose);
				break;
				case 'up':
