...
class giftcontrol extends base {
    ...
    function onadd() {
        if(isset($this->post['realname'])) {
            ...
            $token = explode('-', tcookie('token'));
            $Qid = $token['0'];
     
            //向管理员发送邮件提醒
            $mailsubject = "礼品兑换提示邮件(请勿回复)"; //邮件主题
            if(!$gift["gift_card"]){  //实物兑换
            	...
            	$mailbody .= "<div style='font-size:14px; color:#666; padding-top:10px;'>3Q-ID:".$Qid."于".date("Y年m月d日h时i分",time())."在3Q系统中兑换了".$gift['title']."物品</div><br/>";
            	$mailbody .= "<div style='padding-top:25px; font-size:14px; color:#888'>这是一封系统邮件，请勿直接回复。如需帮助，请发送邮件至 <a href='mailto:help@3q.com' target='_blank' style='color:#888; text-decoration:none'>help@3q.com</a></div>";
            	$mailbody .= "</div>";
            	$mailbody .= "</div>";
            }else{  //虚拟卡兑换
            	//向3q发送充值信息
            	$giftPwd = "simba-".$this->user['credit2'].$this->user['credit2']-$gift['credit'].date("Ymd",time());
            	$result = giftCardFill($gift["gift_card"],$giftPwd);
            	//	判断$result结果，进行下一步操作
            	
            	//邮件内容
            	$mailbody .= "<div style='width:700px; padding:5px; font-family:微软雅黑; font-size:12px; background:#bdd4e2; border-radius:5px;'>";
            	$mailbody .= "<div style='background:#fff; padding:20px; border:solid 1px #dedcdc; border-radius:5px;'>";
            	$mailbody .= "<div style='font-size:14px; color:#666; padding-top:10px;'>3Q-ID".$Qid."于".date("Y年m月d日h时i分",time())."在3Q系统中兑换了".$gift['gift_card']."元的点卡</div><br/>";
            	$mailbody .= "<div style='padding-top:25px; font-size:14px; color:#888'>这是一封系统邮件，请勿直接回复。如需帮助，请发送邮件至 <a href='mailto:help@3q.com' target='_blank' style='color:#888; text-decoration:none'>help@3q.com</a></div>";
            	$mailbody .= "</div>";
            	$mailbody .= "</div>";
            }
